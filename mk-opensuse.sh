...
export RELEASE=$(echo $1 | sed "s/${DISTRIBUTION}-//")
export ARCH=$2
export CONTAINER=$3
export PACKAGE=$4
...
# ./opensuse/vagrant-lxc-fixes.sh ${DISTRIBUTION} ${RELEASE} ${ARCH} ${CONTAINER}
./opensuse/install-extras.sh ${CONTAINER}
./common/prepare-vagrant-user.sh ${DISTRIBUTION} ${CONTAINER}
./opensuse/clean.sh ${CONTAINER}
